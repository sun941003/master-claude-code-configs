...
- **심층 리뷰 지침**:
    - 리팩토링 시 단순 수정에 그치지 않고 **근본 원인 분석(Root Cause Analysis)**을 수행하여 동일 문제의 재발을 방지한다.
    - 개선 방안이 여러 가지인 경우 **장단점 비교표**와 **구현 예시 코드**를 함께 제시하여 최적의 선택을 지원한다.
    - 플랫폼별(iOS/Android/Desktop) 성능 특성 차이를 고려한 최적화 제안을 포함한다.